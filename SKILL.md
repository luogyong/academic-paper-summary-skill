...
| **图表自动提取**     | 自动提取论文中的图表为 PNG，在总结中以 `![[figure.png]]` 嵌入链接        |
| **教材/书籍处理**    | 自动识别书籍，按章节拆分总结，生成全书索引，逐章 Wiki 化                  |
...
用户输入
  ...
  │     ↓
  │   [可选] MinerU 提取全文 + 图表导出（PDF/Word/图片）
  │     ↓
  │   FOCUS 阶段A：逐段抽取（含细节、引文、指标、图表）
  │     ↓
  │   FOCUS 阶段B：清理、结构化、添加概述
  │     ↓
  │   Obsidian 格式化（frontmatter + wiki-link + 图表嵌入）
  │     ↓
  ...
  └── 教材/书籍 → 分章节模式
      ...
      MinerU 提取全书 → 按章节拆分
        ↓
      逐章执行 FOCUS 阶段A+B（每章独立 .md）
        ↓
      生成全书索引（章节列表 + 跨章节主题）
        ↓
      [可选] 逐章 Wiki 化（全书 hub + 章节子页面）
```
...
3. **文档类型识别**
   在 MinerU 提取完成后，检测是否为教材/书籍（非研究论文）：
   - **长度信号**：提取后文本 > 30,000 字（约 50 页以上）
   - **结构信号**：检测章节标题模式：
     * 中文：`第[一二三四五六七八九十\d]+章`、`第[一二三四五六七八九十\d]+节`
     * 英文：`Chapter \d+`、`Part \d+`、`Section \d+\.\d+`
     * 计数：如果匹配到 ≥ 3 个章节标题 → 书籍特征
   - **内容信号**：
     * 缺少论文特征章节：Abstract、Experiments、Results、Related Work
     * 有书籍特征：Preface/前言、目录/Table of Contents、习题/Exercises、参考文献/Bibliography、附录/Appendix、索引/Index
   - **用户声明**：用户明确提及"这本书"、"教材"、"课本"、"textbook"
   - **判定结果**：
     * 如果满足长度 + 结构 + 内容信号（任一 ≥2）→ **自动进入分章节处理模式**（详见 §教材/书籍分章节处理）
     * 如果仅满足 1 项 → 向用户确认是否为书籍
     * 如果都不满足 → 继续标准 FOCUS 论文流程
...
5. **图表提取**
   对论文中的每个图表，执行以下操作：
   - **识别图表**：从 MinerU 输出或原文中定位所有 Figure 和 Table
   - **提取图片**：
     * MinerU JSON 输出时：解析 `figures` 字段，提取 base64 图片数据
     * MinerU MD 输出时：检查 `images/` 子目录中是否有提取的图片
     * 将图片保存到 `{output_dir}/figures/`，命名：`fig_NN.png`（图）或 `tab_NN.png`（表截图）
   - **记录元数据**：
     * 图表编号（如 Fig. 1、Table 2）
     * 原始标题（caption）
     * 在原文中的位置（章节）
   - **Fallback**：如果无法提取图片数据，只记录标题文字 + `[图表未提取]` 标注
...
> **适用场景**：当输入材料是一本教材、专著或书籍（而非研究论文）时，自动按章节拆分，逐章总结并 Wiki 化，生成全书索引。适合教材研读、专著笔记、课本知识图谱构建等场景。

### 模式识别

**判断条件**（在步骤 0.3 中自动检测）：
1. 提取后文本 > 30,000 字（约 50 页以上）
2. 匹配到 ≥ 3 个章节标题（`第X章`、`Chapter N`、`Part N`）
3. 缺少论文特征章节，有书籍特征（前言、目录、习题等）
4. 用户声明"这本书"、"教材"、"课本"

**满足 ≥2 项** → 自动进入分章节模式。仅满足 1 项 → 向用户确认。
...
建议操作：按章节拆分，逐章总结并 Wiki 化。
每章生成独立的笔记页面，最后生成全书索引。

是否继续？[Y/n]
  [也可以选择只处理部分章节，输入章节编号如 1-5]
```

**用户选项**：
- `Y` — 处理全部章节
- `N` — 取消，回退到标准模式（整本书作为一篇论文处理）
- `1-5` — 只处理指定章节范围
- `1,3,5,8-10` — 处理指定章节列表
...
```bash
# 提取全书为 Markdown
mineru-open-api extract "教材文件路径" -o ./_book_output/ -f md

# 同时提取 JSON 以获取图表
mineru-open-api extract "教材文件路径" -o ./_book_output/ -f json
```

**按章节拆分全文**：

1. 读取 MinerU 输出的完整 Markdown
2. 按章节标题（H1 级别）拆分为 N 个片段：
   - 匹配模式（中文）：`^# 第[一二三四五六七八九十\d]+章\s`
   - 匹配模式（英文）：`^# Chapter \d+`
   - 匹配模式（通用）：`^# \d+\.\s`
3. 每个片段 = 一个章节的完整内容
4. 前言/序言/目录等前置内容 → 单独作为"前置"章节
5. 附录/参考文献等后置内容 → 单独作为"附录"章节
...
│   ├── fig_01.png
│   └── ...
└── _book_progress.json           # 处理进度（支持中断恢复）
```
...
|------|---------|-------------|
| 粒度 | 整篇论文 | 单个章节 |
| 提取重点 | 创新点、实验、对比 | 概念讲解、定理推导、例题、知识结构 |
| 公式处理 | 保留关键公式 | **保留所有公式**（教材公式都是核心） |
| 图表处理 | 选择性提取 | **全部提取**（图表是教学核心） |
| 跨引用 | 引用其他论文 | **引用其他章节**（章间关联） |
| 前置依赖 | 一般无 | 标注前置知识/章节依赖 |
...
authors: [...]
year: 2024
tags: [book-chapter, 机器学习, 线性模型]
prerequisites: ["第2章：数学基础"]
created: 2026-06-06
---

# 第3章：线性模型

> **来源**：《[[机器学习导论_INDEX]]》第3章（42 页）
> **前置知识**：[[Ch02_数学基础]] — 矩阵运算、梯度下降
...
────────────────────────────────────────
  ✓ 章节拆分完成
  ✓ 阶段A：提取 15 个关键点，3 张图表
  ✓ 阶段B：结构化完成（7 个主题组）
  ✓ 已保存：chapters/Ch01_绪论_2026-06-06.md

────────────────────────────────────────
[2/14] 第2章：数学基础（12,300 字）
────────────────────────────────────────
  ✓ 章节拆分完成
  ✓ 阶段A：提取 22 个关键点，8 个公式，5 张图表
  ✓ 阶段B：结构化完成（7 个主题组）
  ✓ 已保存：chapters/Ch02_数学基础_2026-06-06.md
...
```

**容错与中断恢复**：
- 单章 FOCUS 失败：跳过，记录到失败列表
- 中断恢复：读取 `_book_progress.json`，从断点继续
- 每章处理完立即保存（不等到全部完成）
...
在所有章节处理完毕后，生成 `{书名}_INDEX_{日期}.md`：
...
| 章节 | 标题 | 核心内容 | 页数 | 笔记 |
|------|------|---------|------|------|
| 第1章 | 绪论 | 机器学习发展史、基本概念、全书框架 | 28 | [[Ch01_绪论_2026-06-06]] |
| 第2章 | 数学基础 | 线性代数、概率论、信息论复习 | 35 | [[Ch02_数学基础_2026-06-06]] |
| 第3章 | 线性模型 | 线性回归、逻辑回归、正则化 | 42 | [[Ch03_线性模型_2026-06-06]] |
| ... | ... | ... | ... | ... |
| 第14章 | 强化学习 | MDP、Q-learning、Policy Gradient | 30 | [[Ch14_强化学习_2026-06-06]] |
...
| 编号 | 标题 | 来源章节 | 图片 |
|------|------|---------|------|
| Fig 1 | ML 发展历程 | 第1章 | ![[figures/fig_01.png]] |
| Fig 2 | 梯度下降示意 | 第2章 | ![[figures/fig_02.png]] |
| ... | ... | ... | ... |
...
1. **速通路线**（2-3 天）：第1章 → 第2章 → 第3章 → 第5章 → 第8章
2. **标准路线**（1-2 周）：按顺序 1→14
3. **专题路线 — 深度学习方向**：第1-2章 → 第5章 → 第8章 → 第11-13章
...
**全书级别**：
1. 全书索引页保存到 `wiki/sources/`
2. 创建 `wiki/entities/{书名}.md` 作为本书实体页
3. 创建 `wiki/concepts/{书名}-核心概念.md` 汇总全书核心概念

**每章级别**：
1. 每章页面保存到 `wiki/sources/{书名}/chapters/`
2. 每章提取的关键实体和概念 → 创建/更新 `wiki/entities/` 和 `wiki/concepts/`
3. 自动建立章间 `[[wiki-link]]`：前置章节 ← 后续章节

**索引更新**：
- `wiki/index.md`：追加全书索引链接 + 每章链接
- `wiki/hot.md`：记录书籍处理上下文

```
=== Wiki 化进度 ===
✓ 全书索引：wiki/sources/机器学习导论_INDEX_2026-06-06.md
✓ 实体页：wiki/entities/机器学习导论.md
✓ 章节子页面：wiki/sources/机器学习导论/chapters/ (14 个文件)
✓ 新建实体：5 个（作者、出版社等）
✓ 新建概念：18 个（核心算法和理论）
✓ 索引已更新：wiki/index.md
✓ 热缓存已更新：wiki/hot.md
...
快速浏览：
1. 打开 机器学习导论_INDEX.md 查看全书索引
2. 在 Obsidian 图谱视图中探索章间关联
3. 按推荐阅读路线开始研读
```
...
| 错误类型 | 处理方式 |
|---------|---------|
| **章节拆分失败**（找不到章节标题） | 降级为整本书单篇处理 |
| **某章提取为空** | 标记为"内容缺失"，记录到失败列表 |
| **章间交叉引用解析失败** | 不影响主流程，跳过该章的交叉引用 |
| **全书索引生成失败** | 手动汇总每章信息生成简化版索引 |
| **用户选择部分章节** | 仅处理指定章节，索引中标注"部分处理" |
...
**处理方式**：
- 自动检测文档类型（长度 + 章节结构 + 内容特征）
- 进入分章节处理模式（§教材/书籍分章节处理），按章节拆分为独立单元
- 每章独立 FOCUS 处理，输出格式针对教学内容调整（保留全部公式、图表、例题）
- 生成全书索引（章节列表 + 跨章节主题分析 + 推荐阅读路线）
- 支持部分章节处理（用户指定章节范围）
- 每章独立 Wiki 化，全书索引作为知识图谱 hub

**适用子场景**：

| 子场景 | 处理方式 | 说明 |
|--------|---------|------|
| 完整教材（>10 章） | 全部逐章处理 | 每 5 章暂停确认一次 |
| 部分章节研读 | 用户指定章节（如 `3-8`） | 仅处理指定范围 |
| 中文教材 | 识别 `第X章` 拆分 | 简体/繁体均支持 |
| 英文教科书 | 识别 `Chapter N` 拆分 | 输出仍为中文 |
| 论文集/多作者专著 | 每章视为独立论文 | 索引中标注各章作者 |
...
**症状**：明显的教材/书籍被当作研究论文处理

**解决**：
1. 确认文件大小足够大（>50 页的 PDF）
2. 检查 PDF 是否包含清晰的章节标题（`第X章`、`Chapter N`）
3. 手动触发：在命令中明确说明"这本书"、"教材"等关键词
4. 如果 MinerU 提取后文字少于 30,000 字，可能是扫描质量差或图片型 PDF

---

### 问题：章节拆分不正确

**症状**：章节拆分后内容错位、章节数量不对、某些章节为空

**解决**：
1. 检查原文的章节标题格式（`#` 级别的标题是否被 MinerU 正确识别）
2. 如果原 PDF 使用非标准章节标题，手动指定匹配模式
3. 使用 `--chapters` 参数手动指定章节起止页码
4. 降级方案：将整本书作为单篇论文处理
...
**解决**：
1. 确认 MinerU 输出包含 `images/` 子目录或 JSON 中的 `figures` 字段
2. 如果 PDF 是扫描版，图表可能嵌入为整页图片而非独立图表
3. 检查 `-f json` 输出格式中的图片区域坐标是否正确
4. 降级方案：在章节笔记中记录图表标题和描述，不嵌入图片
